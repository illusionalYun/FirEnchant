...
public class NMSHandlerImpl implements NMSHandler {
    ...
    public int getAnvilLevel(Player player, ItemStack itemStack, ItemStack enchantBookItemStack) {
        ...
        for (Object2IntMap.Entry<Holder<Enchantment>> entry : enchantments.entrySet()) {
            ...
            for (Holder<net.minecraft.world.item.enchantment.Enchantment> h : mutable.keySet()) {
                if (!h.equals(holder) && !net.minecraft.world.item.enchantment.Enchantment.areCompatible(holder, h)) {
                    canEnchant = false;
                    i++;
                }
            }
